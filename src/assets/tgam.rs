...
impl<'a> TryRead<'a> for Tgam<'a> {
    fn try_read(bytes: &'a [u8], _ctx: ()) -> byte::Result<(Self, usize)> {
        ...
        let header: &'a [u8] = bytes.read_with(offset, ctx::Bytes::Len(3))?;
        if header != b"AGT" {
        ...
        let tga_bytes: &[u8] = bytes.read_with(offset, ctx::Bytes::Len(tga_size as usize))?;
        let mask_bytes: &[u8] = bytes.read_with(offset, ctx::Bytes::Len(mask_size as usize))?;
